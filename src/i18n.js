...
export const translations = {
  ko: {
    ...
    fishingShopButtonLabel: '🛒 상점',
    fishingShopTitle: (p) => `🛒 낚시 장비 상점 (보유 코인: ${p.coins})`,
    fishingShopTabRod: '낚싯대',
    fishingShopTabBoat: '배',
    fishingShopTabHook: '바늘',
    fishingShopDepthLabel: (p) => `최대 깊이 ${p.depth}`,
    fishingShopPastTier: '✅ 통과함',
    fishingShopLocked: '🔒 이전 단계 먼저 구매하세요',
    fishingHookWhite: '흰색 바늘',
    fishingHookBlue: '파란 바늘',
    fishingHookRed: '빨간 바늘',
    fishingHookRainbow: '무지개 바늘',
    fishingHookGold: '골드 바늘',
    fishingHookRadiusLabel: (p) => `반경 ${p.radius}`,
    fishingBoatRowboat: '나룻배',
    fishingBoatMotorboat: '통통배',
    fishingBoatBigboat: '좀 더 큰 배',
    fishingBoatFerry: '여객선',
    fishingBoatWarship: '군함',
    fishingBoatGoldenYacht: '황금 유람선',
    fishingBoatClaim: '🎁 조건 달성! 탭해서 획득',
    fishingBoatLockedRounds: (p) => `🔒 낚시 ${p.required}라운드 플레이 (${p.current}/${p.required})`,
    fishingBoatLockedRare: (p) => `🔒 희귀 물고기 ${p.required}마리 포획 (${p.current}/${p.required})`,
    ...
    blockShopButtonLabel: '🛒 상점',
    blockShopTitle: (p) => `🛒 블록 테마 상점 (보유 코인: ${p.coins})`,
    blockThemeDefaultName: '기본 테마',
    blockThemeNeonName: '네온 테마',
    blockThemePastelName: '파스텔 테마',
    ...
    candyReadyLevelLabel: (p) => `레벨 ${p.level}`,
    candyObjectiveScoreDesc: (p) => `목표: 점수 ${p.target}점 이상`,
    candyObjectiveCollectDesc: (p) => `목표: ${p.tileName} ${p.count}개 모으기`,
    candyReadyStats: (p) => `보유 코인: ${p.coins}   최고 점수: ${p.best}`,
    candyReadyCta: '👉 탭하거나 스페이스바로 시작',
    candyShopButtonLabel: '🛒 상점',
    candyShopTitle: (p) => `🛒 타일 테마 상점 (보유 코인: ${p.coins})`,
    candyThemeDefaultName: '기본 테마',
    candyThemeNeonName: '네온 테마',
    candyThemePastelName: '파스텔 테마',
    candyMovesLeft: (p) => `이동 ${p.moves}회 남음`,
    candyScoreLabel: (p) => `점수: ${p.score}`,
    candyObjectiveScoreProgress: (p) => `목표 점수: ${p.target}`,
    candyObjectiveCollectProgress: (p) => `${p.have}/${p.count}`,
    candyLevelClearTitle: (p) => `🎉 레벨 ${p.level} 클리어!`,
    candyAllLevelsClear: '🏆 모든 레벨을 클리어했어요!',
    candyLevelFailTitle: (p) => `😢 레벨 ${p.level} 실패...`,
    candyNextLevel: '▶ 다음 레벨',
    candyRetry: '🔄 다시 도전',
    candyCoinsEarned: (p) => `+${p.coins}코인`,
    candyTileNameRocket: '로켓',
    candyTileNameFish: '물고기',
    candyTileNameCoin: '코인',
    candyTileNameShield: '실드',
    candyTileNameStar: '별',
  },
  en: {
    ...
    fishingShopButtonLabel: '🛒 Shop',
    fishingShopTitle: (p) => `🛒 Fishing Gear Shop (Coins: ${p.coins})`,
    fishingShopTabRod: 'Rod',
    fishingShopTabBoat: 'Boat',
    fishingShopTabHook: 'Hook',
    fishingShopDepthLabel: (p) => `Max depth ${p.depth}`,
    fishingShopPastTier: '✅ Cleared',
    fishingShopLocked: '🔒 Buy the previous tier first',
    fishingHookWhite: 'White Hook',
    fishingHookBlue: 'Blue Hook',
    fishingHookRed: 'Red Hook',
    fishingHookRainbow: 'Rainbow Hook',
    fishingHookGold: 'Gold Hook',
    fishingHookRadiusLabel: (p) => `Radius ${p.radius}`,
    fishingBoatRowboat: 'Rowboat',
    fishingBoatMotorboat: 'Motorboat',
    fishingBoatBigboat: 'Bigger Boat',
    fishingBoatFerry: 'Ferry',
    fishingBoatWarship: 'Warship',
    fishingBoatGoldenYacht: 'Golden Yacht',
    fishingBoatClaim: '🎁 Unlocked! Tap to claim',
    fishingBoatLockedRounds: (p) => `🔒 Play ${p.required} rounds (${p.current}/${p.required})`,
    fishingBoatLockedRare: (p) => `🔒 Catch ${p.required} rare fish (${p.current}/${p.required})`,
    ...
    blockShopButtonLabel: '🛒 Shop',
    blockShopTitle: (p) => `🛒 Block Theme Shop (Coins: ${p.coins})`,
    blockThemeDefaultName: 'Default Theme',
    blockThemeNeonName: 'Neon Theme',
    blockThemePastelName: 'Pastel Theme',
    ...
    candyReadyLevelLabel: (p) => `Level ${p.level}`,
    candyObjectiveScoreDesc: (p) => `Goal: score ${p.target}+ points`,
    candyObjectiveCollectDesc: (p) => `Goal: collect ${p.count} ${p.tileName}`,
    candyReadyStats: (p) => `Coins: ${p.coins}   Best score: ${p.best}`,
    candyReadyCta: '👉 Tap or press Space to start',
    candyShopButtonLabel: '🛒 Shop',
    candyShopTitle: (p) => `🛒 Tile Theme Shop (Coins: ${p.coins})`,
    candyThemeDefaultName: 'Default Theme',
    candyThemeNeonName: 'Neon Theme',
    candyThemePastelName: 'Pastel Theme',
    candyMovesLeft: (p) => `${p.moves} moves left`,
    candyScoreLabel: (p) => `Score: ${p.score}`,
    candyObjectiveScoreProgress: (p) => `Goal score: ${p.target}`,
    candyObjectiveCollectProgress: (p) => `${p.have}/${p.count}`,
    candyLevelClearTitle: (p) => `🎉 Level ${p.level} clear!`,
    candyAllLevelsClear: '🏆 All levels cleared!',
    candyLevelFailTitle: (p) => `😢 Level ${p.level} failed...`,
    candyNextLevel: '▶ Next Level',
    candyRetry: '🔄 Retry',
    candyCoinsEarned: (p) => `+${p.coins} coins`,
    candyTileNameRocket: 'Rocket',
    candyTileNameFish: 'Fish',
    candyTileNameCoin: 'Coin',
    candyTileNameShield: 'Shield',
    candyTileNameStar: 'Star',
  },
}
